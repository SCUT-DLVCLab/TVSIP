...
class BinaryMetirc(object):
    ...
    def show_result(self, mode):
        ...
        print(tb_pix)
        print(tb_img)
        with open(osp.join(self.save_dir, 'ans_{}.txt'.format(self.method)), 'w') as fp:
            fp.write(str(tb))
            fp.writelines('\n\n')
            fp.write(str(tb_pix))
            fp.writelines('\n\n')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="RTM evaluation")
    parser.add_argument('--module', type=str, default='Fusion', 
                    help='Module name')
    ...
    module = args.module
    pred_dir = f'results/Locator/{module}/Test/pred'
    gt_dir = './datasets/RealTextManipulation/SegmentationClass'
